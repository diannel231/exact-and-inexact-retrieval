    ...
    def buildIndex(self):
        ...
        for each_file in doc_files:
            with open(self.path + "/" + each_file) as file:
                ...
                text = file.read()
            # filter out stop list words
            text_array = self.filter_words_with_stoplist(text)
            docId = docIdDictionary[each_file]
            # store tf values for each term on per document basis
            self.tf_dict[docId] = self.build_tf_dict(text_array)
